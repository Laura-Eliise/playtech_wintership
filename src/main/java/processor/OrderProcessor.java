    ...
    private void handleOrder(String[] order, Player player, List<Match> matches) {
        try {
            switch (order[1]) {
                ...
                case "BET" -> matches
                        .get(ListUtils.findFirstIndex(matches, match -> order[2].equals(match.id)))
                        .bet(player, Integer.parseInt(order[3]), MatchResult.fromString(order[4]));
            }
        } catch (Exception e) {
            player.illegalOperation = String.join(",", order);
        }
    }
